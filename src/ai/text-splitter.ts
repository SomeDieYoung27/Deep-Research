   ...
   export class RecursiveCharacterTextSplitter 
    ...
    implements RecursiveCharacterTextSplitterParams
    {
        ...
        constructor(fields?:Partial<RecursiveCharacterTextSplitterParams>){
            super(fields);
            this.seperators = fields?.seperators ??this.seperators;
        }
        splitText(text:string) : string[] {
            const finalChunks : string[] = [];
            //Get an appropriate seperator to use
            let separator : string = this.seperators[this.seperators.length-1];
            for(const s of this.seperators){
                if(s === ''){
                    separator = s;
                    break;
                }
            ...
            //Now we have the seperator,split the text
            let splits : string[];
            if(separator){
